...
def test_is_profitable():
    ...
    s_bybit = 0.005  # Bybit 滑点 0.5%
    U = 100  # 投入 100 USDT
    w = 0.5  # 提币费 0.5 USDT
    
    result = is_profitable(B, J, s_bybit, U, w)
    print(f"测试1 - 盈利情况:")
    print(f"  B={B}, J={J}, s_bybit={s_bybit}, U={U}, w={w}")
    ...
    result2 = is_profitable(B, J, s_bybit, U, w)
    print(f"测试2 - 亏损情况:")
    print(f"  B={B}, J={J}, s_bybit={s_bybit}, U={U}, w={w}")
    ...
    B = 1.0
    J = 1.02  # 接近临界点
    result3 = is_profitable(B, J, s_bybit, U, w)
    print(f"测试3 - 临界情况:")
    print(f"  B={B}, J={J}, s_bybit={s_bybit}, U={U}, w={w}")
    print(f"  结果: {result3}")
    print()
...
def test_get_max_slippage():
    ...
    # 验证：使用这个最大滑点应该刚好盈亏平衡
    result = is_profitable(B, J, max_slip, U, w)
